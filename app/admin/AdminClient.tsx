  ...
  return (
    <div className="p-8 max-w-[1400px]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-8">
        <div className="bg-card text-card-foreground shadow-sm border border-red-500/30 rounded-2xl p-6 relative">
          <h3 className="font-bold uppercase text-xs text-red-500 tracking-wider mb-6">Назначение Ролей</h3>
          
          <div className="space-y-4">
            <div className="relative">
              <label className="block text-[10px] font-bold uppercase text-muted-foreground mb-1.5">Никнейм пользователя</label>
              <input
                type="text" value={targetUser} onChange={e => handleSearch(e.target.value)}
                className="w-full bg-background border border-border rounded-xl px-4 py-3 text-foreground focus:border-red-500 font-sans text-sm"
              />
              {suggestions.length > 0 && (
                <ul className="absolute top-full left-0 right-0 bg-card text-card-foreground shadow-sm border border-border rounded-xl mt-1 overflow-hidden z-50">
                  {suggestions.map(s => (
                    <li key={s} onClick={() => {setTargetUser(s); setSuggestions([]);}} className="px-4 py-2 hover:bg-red-500 hover:text-black dark:text-white cursor-pointer font-sans text-sm">
                      {s}
                    </li>
                  ))}
                </ul>
            ...
            <div className="flex gap-4 pt-4 mt-6">
              ...
              <button className="flex-1 bg-muted border border-border text-muted-foreground font-bold uppercase py-3 rounded-xl hover:bg-zinc-700 text-sm transition-colors">
                Забрать Права
              </button>
            </div>
            <p className="text-[10px] uppercase text-muted-foreground font-sans mt-4">Требуется таблица user_roles.</p>
          </div>
        </div>

        <div className="bg-card text-card-foreground shadow-sm border border-yellow-500/30 rounded-2xl p-6 relative">
          <h3 className="font-bold uppercase text-xs text-yellow-500 tracking-wider mb-6">Финансы</h3>
          
          <div className="space-y-4">
            <button disabled={loading} onClick={handleAccrueInterests} className="w-full bg-yellow-400 text-black dark:text-white font-bold uppercase py-3.5 rounded-xl hover:bg-yellow-500 transition-colors disabled:opacity-50 text-sm flex justify-center items-center gap-2">
              <span>Симуляция времени (Начислить % на вклады)</span>
            </button>
            <p className="text-[10px] uppercase text-muted-foreground font-sans mt-4">Единоразовое начисление текущей % ставки на все срочные счета.</p>
          </div>
        </div>

        <div className="bg-card text-card-foreground shadow-sm border border-blue-500/30 rounded-2xl p-6 relative">
          <h3 className="font-bold text-xs text-blue-500 mb-6 italic">NOVA PAY Тест</h3>
          
          <div className="space-y-4">
            <a href="/novapay/pay?merchant=Вавилон_Казна&amount=10&item=Тестовая Лицензия&callback=/admin" className="block w-full bg-blue-600 text-foreground font-bold py-3.5 rounded-xl hover:bg-blue-700 transition-colors text-center text-sm shadow-[0_4px_16px_rgba(37,99,235,0.2)]">
      ...
      <div className="bg-card text-card-foreground shadow-sm border border-red-500/30 rounded-2xl p-6 relative">
        ...
        <pre className="bg-background/50 border border-border p-4 rounded-xl text-xs text-green-400 overflow-x-auto">
