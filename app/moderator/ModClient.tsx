...
export default function ModClient({ user, stats }: { user: any, stats?: any }) {
  ...
  return (
    <div className="p-8 max-w-[1400px]">
      ...
      {stats && (
        <div className="bg-yellow-400/10 rounded-2xl p-6 border border-yellow-400/20 grid grid-cols-2 md:grid-cols-5 gap-4 items-center mb-8">
          <div className="col-span-2 md:col-span-1">
            <p className="text-[10px] text-muted-foreground font-bold uppercase  mb-2">Казна Сервера</p>
            <div className="flex flex-col gap-1">
              <p className="text-sm text-foreground font-sans font-bold flex items-center gap-1.5">
          ...
          <div>
            <p className="text-[10px] text-muted-foreground font-bold uppercase  mb-1">Свободные Алмазы</p>
            <p className="text-xl text-foreground font-sans font-bold flex items-center">
              {stats.activeCirculation} <img src={SECRET_TEXTURES.diamond} className="w-4 h-4 ml-1 opacity-50" alt="diamond" />
            </p>
          </div>
          <div>
            <p className="text-[10px] text-muted-foreground font-bold uppercase  mb-1">Выдано Кредитов</p>
            <p className="text-xl text-red-500 font-sans font-bold flex items-center">
              {stats.issuedCredits} <img src={SECRET_TEXTURES.diamond} className="w-4 h-4 ml-1 opacity-50" alt="diamond" />
            </p>
          </div>
          <div>
            <p className="text-[10px] text-muted-foreground font-bold uppercase  mb-1">Депозиты Игроков</p>
            <p className="text-xl text-green-500 font-sans font-bold flex items-center">
              {stats.securedDeposits} <img src={SECRET_TEXTURES.diamond} className="w-4 h-4 ml-1 opacity-50" alt="diamond" />
            </p>
          </div>
          <div>
            <p className="text-[10px] text-muted-foreground font-bold uppercase  mb-1">Активные Игроки</p>
            <p className="text-xl text-foreground font-sans font-bold">{stats.totalPlayers}</p>
          </div>
        </div>
      )}

      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="bg-card text-card-foreground shadow-sm border border-amber-500/30 rounded-2xl p-6">
          <h3 className="font-bold uppercase text-xs text-amber-500 tracking-wider mb-6">Управление Балансом Игроков</h3>
          
          <div className="space-y-4">
            <div className="relative">
              <label className="block text-[10px] font-bold uppercase text-muted-foreground mb-1.5">Никнейм игрока</label>
              <input
                type="text" value={targetUser} onChange={e => handleSearch(e.target.value)}
                className="w-full bg-background border border-border rounded-xl px-4 py-3 text-foreground focus:border-amber-500 font-sans text-sm"
              />
              {suggestions.length > 0 && (
                <ul className="absolute top-full left-0 right-0 bg-card text-card-foreground shadow-sm border border-border rounded-xl mt-1 overflow-hidden z-50">
                  {suggestions.map(s => (
                    <li key={s} onClick={() => {setTargetUser(s); setSuggestions([]);}} className="px-4 py-2 hover:bg-amber-500 hover:text-black dark:text-white cursor-pointer font-sans text-sm">
                      {s}
                    </li>
                  ))}
                </ul>
        ...
        <div className="bg-card text-card-foreground shadow-sm border border-amber-500/30 rounded-2xl p-6">
           ...
           {requests.length === 0 ? <p className="text-sm font-sans text-muted-foreground">Нет новых заявок.</p> : (
             <div className="space-y-4 max-h-[400px] overflow-y-auto">
               {requests.map(r => {
                 let detailsMarkup = <p className="text-xs text-muted-foreground font-sans mt-1">Детали: {r.details}</p>;
                 let parsed: any = null;
                 try {
                   ...
                   if (parsed && typeof parsed === 'object' && parsed.payback) {
                     ...
                     detailsMarkup = (
                       <div className="mt-2 space-y-1 bg-background/40 p-3 rounded-lg border border-red-500/20 text-xs text-muted-foreground font-sans">
                 ...
                 return (
                 <div key={r.id} className="bg-background/50 border border-border p-4 rounded-xl flex flex-col gap-3">
                   <div>
                     <p className="text-sm font-bold text-foreground mb-1"><span className="text-amber-500 uppercase mr-2">{r.req_type}</span> от {r.users?.nick}</p>
                     <p className="text-xs text-muted-foreground font-sans flex items-center gap-1">Сумма: {r.amount} 
                   ...
                   {r.req_type === 'withdraw' || r.req_type === 'withdraw_item' ? (
                     <div className="flex gap-2 items-center w-full mt-2 border-t border-border pt-3">
                       <input
                         type="text"
                         placeholder="Код от игрока"
                         value={withdrawCodes[r.id] || ''}
                         onChange={(e) => setWithdrawCodes({...withdrawCodes, [r.id]: e.target.value})}
                         className="flex-1 bg-background border border-border px-3 py-1.5 rounded text-xs text-foreground"
                       />
                       <button onClick={() => handleConfirmWithdraw(r.id)} className="bg-amber-500 text-black dark:text-white text-xs px-3 py-1.5 rounded disabled:opacity-50 hover:bg-amber-400">
                         Выдать
                       </button>
